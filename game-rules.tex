...
\item There are 25 scoring zones in the arena, arranged in a 5x5 grid. The outer 16 are known as the ``outer ring'', the inner 8 are known as the ``inner ring'', and the most central zone is known as the ``pit''. The inner ring is elevated from the floor by 100 mm $\pm$ 5 mm, so to access the pit robots will need to climb onto it.
...
\item A token is considered to be `in` a zone iff (TODO: edges) at least two vertices of the token are in contact with the floor area inside the zone. The floor area inside the zone is bounded by the edges of the raised inner ring, and the inside edge of the coloured tape marking the zone.
